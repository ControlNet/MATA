...
class GroundingDino(BaseModel):
    ...
    def __init__(self, gpu_number=0):
        ...
        path_checkpoint = str(get_root_folder(
        ) / "pretrained_models" / "grounding_dino" / "groundingdino_swinb_cogcoor.pth")
        config_file = str(get_root_folder() / "module_repos" / "Grounded-Segment-Anything" /
    ...
    def transfer_boxes_format(self, boxes, height, width):
        ...
        for i in range(len(boxes)):
            box = boxes[i]
            transfered_box = [[int(box[0]), int(box[1])], [
                int(box[2]), int(box[3])]]
    ...
    def forward(self, input_image, grounding_caption, box_threshold=None, text_threshold=0.25):
        ...
        if box_threshold is None:
            box_threshold = Config.base_config["grounding_threshold"]
        input_image = np.asarray(
            input_image.permute(1, 2, 0)*255, dtype=np.uint8)

        img_pil = Image.fromarray(input_image)
        re_width, re_height = img_pil.size
        _, image_tensor = self.image_transform_grounding(img_pil)

        boxes, confidences, _phrases = predict(
            self.gd, image_tensor, grounding_caption, box_threshold, text_threshold, device=self.deivce)

        transfered_boxes = self.transfer_boxes_format(
            boxes, re_height, re_width)
        transfered_boxes = transfered_boxes.reshape(
            transfered_boxes.shape[0], 4)[:, [0, 3, 2, 1]]
...
@module_registry.register("grounding_dino_b")
class GroundingDINO_SwinB_Cogcoor(GroundingDino):
    pass
